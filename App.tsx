...
const App: React.FC = () => {
  ...
  const triggerSpectacularConfetti = () => {
    ...
    const interval: any = setInterval(function() {
      ...
      confetti({ ...defaults, particleCount, origin: { x: 0.5, y: 0.5 }, scalar: 1.5, colors: ['#ff0000', '#ffd700'] });
    }, 200);
  ...
  const VerticalCouplet = ({ text, side }: { text: string; side: 'left' | 'right' }) => (
    <motion.div
      ...
      className={`fixed top-[38.2%] -translate-y-1/2 ${side === 'left' ? 'left-8' : 'right-8'} hidden xl:flex flex-col items-center gap-2 z-20`}
    >
      <div className="w-20 bg-gradient-to-b from-red-600 to-red-800 border-4 border-yellow-500 rounded-xl p-4 shadow-[0_20px_50px_rgba(0,0,0,0.5)] relative">
        ...
        {text.split('').map((char, i) => (
          <div key={i} className="text-5xl text-yellow-400 font-chinese font-bold leading-tight select-none mb-3 text-center drop-shadow-[0_2px_4px_rgba(0,0,0,0.5)]">
            {char}
          </div>
        ))}
      </div>
